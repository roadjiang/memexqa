...
flags.DEFINE_float("start_learning_rate", 0.1, "starting learning rate")
...
def train_model(infile):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    q_len_pl = tf.placeholder(tf.int32, batch_size)
    i_pl = None
    g_pl = tf.placeholder(tf.int32, [batch_size, 3])
    t_pl = tf.placeholder(tf.int32, [batch_size, 1])
    pt_pl = tf.placeholder(tf.int32, [batch_size, train_data.max_window_size])
    at_pl = tf.placeholder(tf.int32, [batch_size, train_data.max_window_size])
    ...
    predictions = models.build_ours(q_pl, q_len_pl, i_pl, g_pl, t_pl, pt_pl, at_pl, len(train_data.vocabulary), train_data._num_classes)
    ...
    for _ in xrange(int(1e08)):
      ...
      feed_dict = {q_pl:batch_data["Qs"], labels_pl:batch_binary_labels,
                   g_pl:batch_data["Gs"], t_pl:batch_data["Ts"], pt_pl:batch_data["PTs"], 
                   at_pl:batch_data["ATs"], q_len_pl:batch_data["Qs_len"]}
