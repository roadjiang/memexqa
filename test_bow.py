...
flags.DEFINE_string("test_dir", "/Users/lujiang/run/bow_ts", "Testing output directory")
...
def test_model(infile):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    bow_pl = tf.placeholder(tf.float32, [batch_size, len(test_data.vocabulary)])
    
    labels_pl = tf.placeholder(tf.int32, [batch_size, test_data.num_classes])

    # Build a Graph that computes predictions from the inference model.
    predictions = models.build_lr_bow(i_pl, bow_pl, test_data._num_classes)
