...
flags.DEFINE_string("model", "lstm_nolookup", "model_name")
...
def train_model(infile):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    placeholders["PTs_l"] = tf.placeholder(tf.int32, [batch_size,])

    placeholders["labels"] = tf.placeholder(tf.int32, [batch_size, train_data.num_classes])
    placeholders["mask"] = tf.placeholder(tf.float32, [batch_size, train_data.num_classes])
    ...
    for _ in xrange(int(2e04)):
      ...
      mask = batch_data["labels"].copy()
      mask[mask == 0] = 0
      
      feed_dict = {}
      modalities = [t for t in train_data._modalities if t not in ["qids", "labels", "As", "Ts", "ATs"]]
      for k in modalities:
