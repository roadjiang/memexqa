...
flags.DEFINE_string("data_path", "/Users/lujiang/data/memex_dataset/exp/ours_tr.p", "data_path")
flags.DEFINE_string("photo_feat", "/Users/lujiang/data/memex_dataset/exp/photo_feat_pca.p", "photo_feat")
flags.DEFINE_string("model", "memexnet_img", "model_name")
...
def train_model(infile):
  ...
  train_data = reader_ours.DataSet(infile)
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    placeholders = {}
    placeholders["Qs"] = tf.placeholder(tf.int32, [batch_size, 12])
    placeholders["Qs_l"] = tf.placeholder(tf.int32, [batch_size,])

    placeholders["Is"] = tf.placeholder(tf.int32, [batch_size, ])

    placeholders["Gs"] = tf.placeholder(tf.int32, [batch_size, train_data.topn])
    placeholders["Ts"] = tf.placeholder(tf.int32, [batch_size, train_data.topn])
    placeholders["PTs"] = tf.placeholder(tf.int32, [batch_size, train_data.topn])
    ...
    for _ in xrange(int(2e04)):
      ...
      feed_dict = {}
      modalities = [t for t in train_data._modalities if t not in ["qids", "labels", "As", "ATs"]]
      for k in modalities:
      ...
      if global_step_val % (2*epoch_size) == 0:
        # reach an epoch
        ...
        # save model
        saver.save(session, os.path.join(train_dir, "{}.model".format(os.path.basename(train_dir))), global_step=global_step_val)
