...
def text_to_tokens(instring):
  """ parse question words to index them.
  """
  if instring is None: return []
  instring = re.sub(SYMBOLS, " ", instring).lower()
  result = []
  for this_word in nltk.word_tokenize(instring):
    if this_word  in stopword_dict: continue
    this_word = stemmer.stem(this_word)
    result.append(this_word)
  return result


def _entail(long_text, short_text):
  return len(np.intersect1d(long_text, short_text))*1.0/(len(short_text)+1e-6)


def _find_max_classid(meta, mchoices):
  sim_scores = [_entail(meta, k) for k in mchoices]
  max_id = np.argmax(sim_scores)
  if(sim_scores[max_id] > 0.5): return max_id
  else: return -1
  

def build_our_dataset():
  indir = "/Users/lujiang/data/memex_dataset/exp/"
  qa, album = pickle.load(open(os.path.join(indir, "qa_album.p"), "rb"))
  pids, photo_feat = pickle.load(open(os.path.join(indir, "photo_feat_pca.p"), "rb"))
  
  reterival_list = pickle.load(open(os.path.join("/Users/lujiang/data/memex_dataset/retrieval/results", "concept+meta.retrieval.top5.p"), "rb"))
  
  classes, vocabulary, tr_qids, ts_qids = pickle.load(open("/Users/lujiang/data/memex_dataset/exp/class_vob_splits.p", "rb"))
  ...
  pid2aid = {}
  for t in album: 
    for k in t["photo_ids"]: pid2aid[k] = t["album_id"]

  pid2ptitle = {}
  for t in album:
    for p, q in zip(t["photo_ids"],t["photo_titles"]): pid2ptitle[p] = text_to_tokens(q + " " + t["album_title"])
    
    
  pid2where = {}
  for t in album:
    for k in t["photo_ids"]: 
      pid2where[k] = text_to_tokens(t["album_where"])

  
  pid2timestamp = {}
  for t in album:
    for p in t["photo_ids"]: pid2timestamp[p] = text_to_tokens(t["album_when"].replace("on ", "").strip())
  ...
  for t in qa:
    retrieved_pids = [str(k[0]) for k in reterival_list[t["question_id"]]]
    mchoices = [text_to_tokens(k) for k in t["multiple_choices_4"]]
    answer_classes = t["ml_multiple_choice_classes"]
    ...
    for i in xrange(topn):
      cur_id = retrieved_pids[i] 
      cid = _find_max_classid(pid2timestamp[cur_id], mchoices)
      if cid != -1: Ts[i] = answer_classes[cid]
      cid = _find_max_classid(pid2where[cur_id], mchoices)
      if cid != -1: Gs[i] = answer_classes[cid]
      cid = _find_max_classid(pid2ptitle[cur_id], mchoices)
      if cid != -1: PTs[i] = answer_classes[cid]
      
    
#     retrieved_aids = np.unique([pid2aid[k] for k in retrieved_pids])
#     Ts = [aid2album[k]["ml_album_when"] for k in retrieved_aids]
#     Gs = [aid2album[k]["ml_album_where"] for k in retrieved_aids]
#     PTs = [pid2ptitle[k] for k in retrieved_pids]
    
    Is = [np.where(pids==k)[0][0] for k in retrieved_pids][0]
  ...
  #evaluate
  tr_gt = [t["ml_answer_class"] for t in qa if t["question_id"] in tr_qids]
  ts_gt = [t["ml_answer_class"] for t in qa if t["question_id"] in ts_qids]
  ...
  for k in xrange(topn):
    acc_t.append(len(np.where([tr_gt[i] in tr["Ts"][i][0:k+1] for i in xrange(len(tr_gt))])[0])*1.0/len(tr_gt))
    acc_g.append(len(np.where([tr_gt[i] in tr["Gs"][i][0:k+1] for i in xrange(len(tr_gt))])[0])*1.0/len(tr_gt))
    acc_pt.append(len(np.where([tr_gt[i] in tr["PTs"][i][0:k+1] for i in xrange(len(tr_gt))])[0])*1.0/len(tr_gt))
    
  for k in xrange(topn):
    acc_t.append(len(np.where([ts_gt[i] in ts["Ts"][i][0:k+1] for i in xrange(len(ts_gt))])[0])*1.0/len(ts_gt))
    acc_g.append(len(np.where([ts_gt[i] in ts["Gs"][i][0:k+1] for i in xrange(len(ts_gt))])[0])*1.0/len(ts_gt))
    acc_pt.append(len(np.where([ts_gt[i] in ts["PTs"][i][0:k+1] for i in xrange(len(ts_gt))])[0])*1.0/len(ts_gt))
