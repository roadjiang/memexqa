...
def test_model(infile):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    placeholders["Gs"] = tf.placeholder(tf.int32, [batch_size, test_data.topn])
    placeholders["Ts"] = tf.placeholder(tf.int32, [batch_size, test_data.topn])
    placeholders["PTs"] = tf.placeholder(tf.int32, [batch_size, test_data.topn])
    ...
    for _ in xrange(int(1e10)):
      ...
      feed_dict = {}
      modalities = [t for t in test_data._modalities if t not in ["qids", "labels", "As", "ATs"]]
      for k in modalities:
